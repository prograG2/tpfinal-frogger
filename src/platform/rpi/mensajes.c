...
bool renglonIzquierdoLibre(mensaje_t *msj){
    for (int i=0; i<TAM_RENGLON; i++)
        if((msj->renglon)[i].mitad_izq) return false;
    return true;
}
...
mensaje_t mensaje(char* msj, int pos, bool repetir){
    ...
    int i;
    for(i=0; i<longitud_parcial; i++){ //rellena el mensaje por primera vez
        ...
        if(2*CANT_COLUMNAS - j - ancho < 0) break; //lo que me quedaría libre si escribo
...
void moverMensaje(mensaje_t* msj){
    ...
    if(!(msj->repetir_msj) && msj->index >= msj->longitud){ //tengo que repetirlo?
